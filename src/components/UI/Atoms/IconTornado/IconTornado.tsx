...
interface IIconTornado extends IImage {}

const IconTornado: React.FC<IIconTornado> = ({ forceColor, forceOpacity }) => {
  const { theme } = useTheme();

  const iconColor = getImageColor(theme, forceColor);

  const opacityClass = forceOpacity ? `opacity-${forceOpacity}` : "";
  const classList = generateClassNameValue(["icon-tornado", opacityClass]);
