...
interface IIconGear extends IImage {}

const IconGear: React.FC<IIconGear> = ({ forceColor, forceOpacity }) => {
  const { theme } = useTheme();

  const iconColor = getImageColor(theme, forceColor);

  const opacityClass = forceOpacity ? `opacity-${forceOpacity}` : "";
  const classList = generateClassNameValue(["icon-gear", opacityClass]);
